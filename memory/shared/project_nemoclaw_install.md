...
metadata: 
  ...
  modified: 2026-08-09T19:48:53.566Z
---
...
**Still blocking (needs elevation):** Windows Firewall inbound rule for port 11434. WSL cannot reach the host Ollama at the gateway address (172.27.208.1) without it. WSL adapter is `vEthernet (WSL)` and its subnet changes across reboots, so scope the rule to `172.16.0.0/12` rather than a fixed subnet.
