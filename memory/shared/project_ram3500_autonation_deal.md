...
metadata: 
  ...
  modified: 2026-08-03T13:36:32.536Z
---
...
**State as of 2026-08-03:** Kris **countered $90,600** on the gray TG332645 (a token $900 off $91,497). User sent a factual, reasoning-led email + a links/VINs follow-up (both "Re: 2026 Ram 3500 Limited comps", to Kris cc Renato from joshua.a.zayne@hotmail.com, 08/03 ~07:32-07:33), pushing **$87,000** (before tax/title, incl $500 veteran) with named VIN-verified comps: **Koons of Baltimore** CPO 2026 Limited Mega Cab $88,300 (VIN 3C63R3PLXTG278013, cars.com id 45943c5b), **Sam Leman** (Morton IL) 2026 Limited Mega Cab $87,407 (VIN 3C63R3PL1TG191942), plus NEW franchise comps **Earnhardt CJDR (Gilbert AZ) $88,665** and **Vernon Auto Group (TX) $90,528**. Market median for a 2026 Limited Mega Cab ~$88,563 (from the scraper data); gray now ~125 days on lot. Both AutoNation trucks still listed ($91,497 gray / $92,997 white). Awaiting Kris; expected settle ~$88k. All comps in the deal doc `F:\iCloudDrive\Documents\Carvana Ram\Comparable-Limiteds.md`.

**GOTCHA — Outlook stuck-Outbox on COM-created mail:** the 07/20 send sat in the Outbox unsent for a day. Cause: a mail item created via `CreateItem` + **`.Save()`** (the draft/`.Display()` flow) lands in the Outbox WITHOUT the submit flag, so Send/Receive skips it forever (Outlook was online, not Work Offline, no open inspector). Fix that worked: recreate a fresh `CreateItem`, set body/To/CC + `SendUsingAccount`, call **`.Send()`** (not Save), delete the stuck copy, then start SyncObjects. Lesson: to actually SEND from COM use `.Send()`; `.Save()`+`.Display()` only drafts, and if the user hits Send on that draft it can still get stuck. There was a second identical stuck item (07/03 spend email) confirming the pattern; that one was cleared 2026-08-03 (deleted from the Outbox). The 08/03 deal emails sent cleanly via the `.Send()`+SyncObjects flow and are confirmed in Sent Items.
